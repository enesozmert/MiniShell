...
/*   Updated: 2022/12/19 10:08:53 by cyalniz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	token_type_is_keyword(t_rdl *rdl)
{
		...
		&& is_keyword(rdl->keyword_list, keyword))
	{
		...
		rdl->token_type_prop->keyword_id
			= find_keyword_id(rdl->keyword_list,
				rdl->token_type_prop->new_trim);
		free(keyword);
		return (1);
	}
	free(keyword);
...
int	token_type_is_operator(t_rdl *rdl)
{
	...
	if (rdl->token_type_prop->token->len == 1
		...
		&& keyword_in_operator(rdl, rdl->token_type_prop->keyword_id,
			rdl->token_type_prop->token->context[0]))
	{
...
int	token_type_is_valid_identifier(t_rdl *rdl)
{
	...
	if ((rdl->token_type_prop->token->len) > 0
		&& is_identifier(rdl->identifier_list,
			rdl->token_type_prop->token->context)
		&& rdl->token_type_prop->keyword_id == 3)
		return (1);
	return (0);
}
...
{
	if (r->token_type_prop->redir_flag == 1)
		return (0);
	if (r->token_type_prop->token->len == 1
		&& r->token_type_prop->keyword_id == 3
		&& r->token_type_prop->token->context[0] == ' ')
		return (0);
	if (r->token_type_prop->token->len > 0
		&& r->token_type_prop->keyword_id == 3
		&& r->token->next
	...
	if (r->token_type_prop->token->len > 0
		&& ((r->token_type_prop->keyword_id == 3
		&& r->token_type_prop->opr_flag == 2)))
		return (1);
	if (r->token_type_prop->token->len > 0 &&
		(((r->token_type_prop->keyword_id == 3
		&& r->token_type_prop->opr_flag == 2)
		&& (r->token_type_prop->sq_flag == 1 || r->token_type_prop->dq_flag == 1))))
		return (1);
	if (r->token_type_prop->token->len > 0 && r->token_type_prop->keyword_id == 3
		&& (r->token_type_prop->sq_flag == 1 || r->token_type_prop->dq_flag == 1))
		return (1);
	return (0);
}
