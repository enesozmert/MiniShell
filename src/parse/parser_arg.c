...
void parser_arg(t_rdl *rdl)
{
	...
	while (++i < rdl->len + 1)
	{
		...
		if (flag >= rdl->quote_count[k].left && flag < rdl->quote_count[k].right + rdl->quote_count[k].left)
			parser_arg_isoperator(c, &j, rdl);
		if (flag >= rdl->quote_count[k].right + rdl->quote_count[k].left)
			parser_arg_isnotoperator(c, &j, rdl);
...
void parser_arg_split(t_rdl *rdl)
{
	...
	redir_l_c = 0;
	redir_r_c = 0;
	if (ft_strchr(rdl->main_str, '>') != NULL)
	{
		i = -1;
